...
//CRUD (Create, Read, Update, Delete) operations on the underlying MySQL database, interacting with tables such as user, movies, tickets, discount, and review
    ...
    public static void addMovie(Movie m, FileInputStream fis) throws SQLException, IOException {
        ...
        String sql = "INSERT INTO movies(name, av_tickets, price_ticket, image)" +
                "VALUES(?,?,?,?);";
        PreparedStatement st = con.prepareStatement(sql);
    ...
    private static ArrayList<Movie> moviesData(Connection con, String sql) throws SQLException {
        ...
        while (rs.next()) {
            Movie m = new Movie(rs.getString("name"), rs.getInt("av_tickets"), rs.getDouble("price_ticket"), rs.getBlob("image"));
